...
nixos modules; consumed by Lojix's deploy projection path.
...
(with its `DatomicEncode`/`DatomicDecode`) — then author the fact, project
...
cluster owner owns its subdomain under it. The exact Datomic shape is
...
- The deploy pipeline — that's Lojix.
